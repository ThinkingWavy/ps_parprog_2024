...
typedef struct node {
	count_t local_count;
	uint8_t padding[CACHE_LINE_SIZE - sizeof(count_t)];
} node_t;
...
int main(int argc, char** argv) {
	...
	node_t* points_in_circle = calloc(requested_threads, sizeof(node_t));
...
#pragma omp parallel num_threads(requested_threads)
	{
		...
		count_t* local_count = &points_in_circle[thread_num].local_count;
...
#pragma omp for
	...
	for(int i = 0; i < requested_threads; i++) {
		global_points += points_in_circle[i].local_count;
	}
